...
- **CRUD Operations** (Admin only):
  ...
  - Delete transactions
  - View deleted transactions list (deleted items tracker)
...
│ │ └── DeletedTransaction.tsx # Deleted transaction audit/trash view
...
All data is stored locally and never leaves the browser. To reset:

Clear localStorage in browser dev tools, or
localStorage.clear() / localStorage.removeItem('transactions')
...
🎨 Customization
Adding Categories
Edit TransactionCategory type in index.ts and update the categories array in TransactionForm.tsx.

Changing Colors
Modify Tailwind classes in components or update the theme in theme.css.

Adding New Insights
Extend calculation functions in calculations.ts and add new sections to Insights.tsx.

🧪 Testing Scenarios
Test Role-Based UI
Switch to Viewer role - notice "Add Transaction" button disappears
In transaction list, notice action buttons (Edit/Delete) are hidden
Switch to Admin role - all controls reappear
...
Test with role and dark mode preferences
🌟 Future Enhancements
